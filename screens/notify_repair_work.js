...
class Notify_repair_work extends Component {
    ...
    serve = async () => {
        ...
        const { id, name, phone, repair_work, address} = this.state;

        this.setState({
            statusSave: false
        })
        const result = await repairWork.createRepairWork(id, name, phone, repair_work, address);
        if (result === "success") {
    ...
    render() {
        const { id, name, phone, repair_work, address, statusSave } = this.state;
        return (
            <SafeAreaView>
